...
- [ ] **A2-Seiten + Maschinen-Layer deployen, Fakten gegenprüfen (#65):** vier Long-tail-Vergleichsseiten (`/total-commander-for-mac`, `/marta-alternative`, `/forklift-alternative`, `/path-finder-alternative`) plus JSON-LD, OG-Cards, `sitemap.xml`, `robots.txt`, `llms.txt`/`llms-full.txt` sind gebaut, aber **nicht deployed** — `wrangler deploy` aus `.scratch/landing-page/`. Davor: Preise einmal vendor-direkt gegenprüfen, die Remote-Umgebung kommt an binarynights/cocoatech/marta.sh/macupdate nicht heran (403). Zwei konkrete Verdachtsfälle im Issue: ForkLifts Update-Fenster-Lizenz, Path Finders Mehrjahres-Staffel (`.scratch/diptychon-mvp/issues/65-a2-longtail-comparison-pages.md`)
- [ ] **Preismodell klären (#66):** Open Source + bezahlte Features vs. Einmalkauf. Ticket steht auf `needs-info` — das Zahlungssignal aus dem Demand-Test fehlt noch. **Sofort machbar und nötig:** PRD nennt „MIT license", ADR 0007 nennt Einmalkauf — Widerspruch auflösen. Website nennt bis dahin keinen Preis mehr (`.scratch/diptychon-mvp/issues/66-open-core-vs-einmalkauf.md`)
- [ ] **Wedge-Begriff entscheiden (#65):** Vorschlag steht — Gegner heißt **„Update-Fenster"**, Versprechen heißt **„Buy once, updates included. No expiry date."** Hängt am ForkLift-Recheck; erst bestätigen, dann in die Copy. Danach ist ein Erklärstück dazu das nächste zitierfähige Asset.
- [ ] **Outreach neu ansetzen (#65):** Tills Befund — die Roundup-Ziele sind fast alle Eigen-/Vendor-Publikationen, echte Redaktion war nur ein brasilianischer Blog ohne Reichweite. Damit ist A3 in der geplanten Form tot. Nächste Option ist ein **eigener, klar als Diptychon gekennzeichneter Content-Hub** (`/blog` unter der eigenen Domain), nicht ein Netz scheinbar unabhängiger Satelliten-Blogs — siehe Notiz im Issue.
...
## Offen — bei AI
Nächstes: Queue in `.scratch/diptychon-mvp/issues/` (ready-for-agent: #39, #40, #52 batch-rename [eigener Worktree], #57). #65 A2-Long-tail-Seiten gebaut 2026-08-02 (liegt bei Till: Deploy + Vendor-Recheck); der Maschinen-Layer daraus — JSON-LD, sitemap.xml, OG-Cards — ist bewusst offen geblieben und wäre der nächste billige Schritt. #36 Gadgets-lite + #53 abgeschlossen + gemerged 2026-07-15; volle Suite grün auf main `2b8bf08` (210 Unit + 13 UI); #61 als Wedge-Artefakt geschlossen (`pkill testmanagerd` = Recovery).
