...
struct PaneState: Codable, Equatable {
    ...
    /// Which of the three display modes the pane was in (issue 91). Optional, so
    /// pre-91 snapshots keep decoding — those fall back to `briefColumns`.
    ///
    /// A separate field rather than widening `briefColumns`: the column browser has
    /// no column count of its own, and folding a third mode into an `Int?` would mean
    /// a magic value that the next reader has to decode.
