...
/// (Marta brief mode). The grid always fits the visible height exactly and scrolls
/// horizontally. Frames are precomputed once per `prepare()` (O(n) arithmetic — cheap
/// even at 50k); only attributes intersecting the requested rect are materialized,
/// so cell creation stays O(visible).
final class BriefLayout: NSCollectionViewLayout {
    ...
    override func prepare() {
        ...
        rowsPerColumn = max(1, Int(floor(cv.bounds.height / Self.itemHeight)))
        columnWidth = max(80, cv.bounds.width / CGFloat(max(1, columns)))
    ...
    override var collectionViewContentSize: NSSize {
        ...
        return NSSize(width: CGFloat(columnCount) * columnWidth, height: cv.bounds.height)
    }
    ...
    /// Height changes alter `rowsPerColumn` (the whole layout), width changes alter
    /// column width — recompute on any resize.
