...
final class PanelModelRestoreTests: XCTestCase {
    ...
    func testPaneStateCapturesDisplayMode() {
        let m = PanelModel(directory: URL(fileURLWithPath: "/tmp/here", isDirectory: true))
        XCTAssertNil(m.paneState.briefColumns)            // default: table
        m.setBriefColumns(3)
        XCTAssertEqual(m.paneState.briefColumns, 3)
    }

    func testRestoreAppliesBriefMode() {
        let m = PanelModel(directory: URL(fileURLWithPath: "/tmp/start", isDirectory: true))
        m.restore(directory: URL(fileURLWithPath: "/tmp/saved", isDirectory: true),
                  sort: .default, briefColumns: 2)
        XCTAssertEqual(m.displayMode, .brief(columns: 2))
        // The restored count is also what toggling back from table returns to.
        m.toggleBriefView()
        XCTAssertEqual(m.displayMode, .table)
        m.toggleBriefView()
        XCTAssertEqual(m.displayMode, .brief(columns: 2))
    }

    func testRestoreWithoutBriefColumnsStaysTable() {
        let m = PanelModel(directory: URL(fileURLWithPath: "/tmp/start", isDirectory: true))
        m.restore(directory: URL(fileURLWithPath: "/tmp/saved", isDirectory: true), sort: .default)
        XCTAssertEqual(m.displayMode, .table)
    }

    func testToggleRoundTripsAndRemembersColumnCount() {
        let m = PanelModel(directory: URL(fileURLWithPath: "/tmp/start", isDirectory: true))
        m.setBriefColumns(1)
        m.toggleBriefView()                               // → table
        XCTAssertEqual(m.displayMode, .table)
        m.toggleBriefView()                               // → brief, same count
        XCTAssertEqual(m.displayMode, .brief(columns: 1))
    }

    func testSetBriefColumnsClampsToScopedRange() {
        let m = PanelModel(directory: URL(fileURLWithPath: "/tmp/start", isDirectory: true))
        m.setBriefColumns(9)
        XCTAssertEqual(m.displayMode, .brief(columns: 3))
        m.setBriefColumns(0)
        XCTAssertEqual(m.displayMode, .brief(columns: 1))
    }

    func testDisplayModeSurvivesSnapshotRoundTrip() {
        let original = PanelModel(directory: URL(fileURLWithPath: "/tmp/a", isDirectory: true))
        original.setBriefColumns(3)
        let snap = original.paneState

        let restored = PanelModel(directory: URL(fileURLWithPath: "/tmp/other", isDirectory: true))
        restored.restore(directory: URL(fileURLWithPath: snap.directoryPath, isDirectory: true),
                         sort: snap.sort, briefColumns: snap.briefColumns)
        XCTAssertEqual(restored.paneState, snap)
    }
}
