...
**Batch rename (verified 2026-07-11/12):** Diptychon **yes, undoable** (BatchRenameSheet + RenameRule on ⌘R multi-selection, Finder-style modes replace/add/format/case, one undoable Operation, commit `14a9553`; regex deferred to v1.1 — the capture-form wish "regex/EXIF" refers to the advanced version, NOT a missing feature) · Marta **no** ([file-operations reference](https://marta.sh/docs/core/file-operations/) lists only single-item Rename ⇧F6) · ForkLift **yes** (Multi-Rename, release notes) · Nimble Commander **yes** ([Help.md "Batch Rename" section](https://github.com/mikekazakov/nimble-commander/blob/main/Docs/Help.md), Ctrl+M) · Path Finder **yes** (KB article "Batch Renaming"). Correction note: an earlier /vs iteration briefly claimed Diptychon "No" here, inferred from the capture-form wish; codebase check falsified that. The "row Diptychon loses" trust function is carried by the Remote (SFTP/WebDAV) row instead.
